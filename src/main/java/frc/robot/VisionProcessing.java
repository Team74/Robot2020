...
public class VisionProcessing {
    ...
    private static int pixyHorizontalFieldoView = 60;
    private static int pixyVerticalFieldoView = 40;
    private static int resolutionHorizontal = 1296;
    private static int resolutionVertical = 976;
    private static double viewPlaneWidth = 2.0 * Math.tan(pixyHorizontalFieldoView / 2);
    private static double viewPlaneHeight = 2.0 * Math.tan(pixyVerticalFieldoView / 2);



    /**
     * @param pixyData gives us the pixel of the center of the target, and the height and width of the taret
     ...
     */
    public static HashMap<String, Double> calculateDistanceAngle(HashMap<String, Integer> pixyData, HashMap<String, Double> targetData) {
        if(pixyData.containsKey("targets")) {
            return null;
        }
        double normalizedx = (1/(resolutionHorizontal / 2)) * (pixyData.get("m_x") - (((resolutionHorizontal - 1) / 2)));
        double normalizedy = (1/(resolutionVertical / 2)) * ((((resolutionVertical - 1) / 2)) - pixyData.get("m_y"));
